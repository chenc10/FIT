...
char  ReadFileName[128] = "nids_cTos_udp";
char  WriteFileName[128] = "Result.result";
float StepParaVariation = 0;
...
float CoverThreshold = 0.5;
int MEMSIZE = 50000;
int PACKETNUMSIZE = 20;
int method = 1; 
...
typedef struct MostItem
{
	...
	int MaxNum;
		//including the number of Transactions that includes this most frequent item, and this is also  MItem->VisitedTimes
}MostItem;
...
int main(int argc, char * argv[])
{
	...
	//		fprintf(fw,"reach end\n");
		fprintf(stderr,"make_split exit\n");
...
{//calulate the number of Transaction including a certain item
	//fprintf(stderr," enter calculate_support\n");
	...
	{//for a given item, we only need to compare the item at the corresponding position of other Transactions
		...
		//fprintf(stderr," enter normal calculate process\n");
...
{//get all the children of a certain node
	fprintf(stderr," enter make_split\n");
	if(is_matrix(Root))
	{
	...
	fprintf(stderr," exit from step_split_differentlevel\n");
	fprintf(stderr," level: %d\n", Root->TLevel);
	if(is_matrix(CreatedTnode2))
		fprintf(stderr,"is_matrx error\n");
	...
	Tnode * CreatedTnode3;
	fprintf(stderr,"prepare to  enter step_split_samelevel\n");
	CreatedTnode3 = step_split_samelevel(MTree,CreatedTnode2);
	fprintf(stderr," exit from step_split_samelevel\n");
...
{//spliting of the same level
	fprintf(stderr," enter step_split_samelevel\n");
	...
	fprintf(stderr,"%d %d %d\n",Brother->TTransactionNum, PACKETNUMSIZE,1);
	for( int i = 0; i < Brother->TTransactionNum; i ++,fprintf(stderr,"again\n"))
		for( int j = 0; j < PACKETNUMSIZE; j ++){ fprintf(stderr,"Brother->TT.P[j]: %d\n",Brother->TTransaction[i].PacketNum[j]);
			for( int k = 0; k < Brother->TTransaction[i].PacketNum[j]; k ++)
				if(!(Brother->TTransaction[i].ItemArray[j][k].IsVisited))
					{
					fprintf(stderr," enter for for for\n");
					...
					fprintf(stderr,"prepare enter calculate_support\n");
	...
	fprintf(stderr,"already get MMostItem\n");
	if(MMostItem.MItem == NULL)
...
//cc
	...
	fprintf(stderr,"leave step_split_samelevel\n");
	return Created;
}
