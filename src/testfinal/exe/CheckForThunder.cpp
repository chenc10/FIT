...
int check(Transaction * MyTransaction)
{
	for( int i = 0; MyTransaction->PacketNum[i]; i ++)
	{
		...
		if( i > 2)
		{
			if( !strcmp(MyTransaction->ItemArray[i][3].Pload, MyTransaction->ItemArray[i-1][3].Pload) && !strcmp(MyTransaction->ItemArray[i][3].Pload, MyTransaction->ItemArray[i-2][3].Pload) && !strcmp(MyTransaction->ItemArray[i][3].Pload, MyTransaction->ItemArray[i-3][3].Pload))
				return 0;
		}
	}
		fprintf(stderr,"ok\n");
