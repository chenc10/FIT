...
int checkpacket(Feature * MyFeature, Item * ItemArray,int ItemLength);
...
int checkpacket(Feature * MyFeature, Item * ItemArray,int ItemLength)
{
	...
	for ( int i = 0, sign = 1; i < MyFeature->FeatureNum; i ++)
	{
		...
		{
			if( ItemLength <= MyFeature->FeatureItems[i][j].ByteSeqNum)
			{
				sign = 0;
				break;
			}
			ItemArray[MyFeature->FeatureItems[i][j].ByteSeqNum].Pload[2] = '\0';
			if( strcmp(ItemArray[MyFeature->FeatureItems[i][j].ByteSeqNum].Pload, MyFeature->FeatureItems[i][j].Pload))
			{
				sign = 0;
				break;
			}
