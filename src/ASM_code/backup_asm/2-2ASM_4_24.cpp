/* feature description:
	1.commandline:			OK
	2.ordered-feature:		NO
	3.dynamic threshold:		NO
	4.special-feathure handle: 	OK
4.19
*/
...
int hex_asc(char *);
int is_useful(Tnode *, int , int ,int );
int m,n;
...
int is_useful(Tnode * Root, int i, int j, int k)
{
	if(k == 0 && hex_asc(Root->TTransaction[i].ItemArray[j][k].Pload) == 71)
		return 0;
	if(k == 1 && hex_asc(Root->TTransaction[i].ItemArray[j][k].Pload) == 69)
		return 0;
	if(k == 2 && hex_asc(Root->TTransaction[i].ItemArray[j][k].Pload) == 84)
		return 0;
	if(k == 3 && hex_asc(Root->TTransaction[i].ItemArray[j][k].Pload) == 32)
		return 0;
	if(k == 4 && hex_asc(Root->TTransaction[i].ItemArray[j][k].Pload) == 47)
		return 0;
	return 1;
}
