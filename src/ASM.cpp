...
float FThreshold = 0.02; //The first node after spliting covers more than a certain part(FThreshold) of all the transactions in its father
float WThreshold = 0.01; //The first node after spliting covers more than a certain part(WThreshold) of all the transactions in the root node
float DiscardThreshold = 0.3; // The number of transactions discarded when a same-level spliting is made must be limited to a certain threshold(DiscardThreshold, otherwise we refuse to make that split
