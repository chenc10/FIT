...
ip_defrag_stub(struct ip *iph, struct ip **defrag)
{
  ...
  numpack++;
  timenow = 0;
  //fprintf(stderr,"YYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYY\n");
  ...
  omp_set_lock(&tptd_gparams.ipflock);
  if ((*defrag = (struct ip *)ip_defrag((struct ip *) (skb->data), skb))){
	omp_unset_lock(&tptd_gparams.ipflock);
	 return IPF_NEW;
  }
  omp_unset_lock(&tptd_gparams.ipflock);
  return IPF_ISF;
...
void TPTD_Init_Plugin_Ipfragment(TPTD_Modules *module){

	strcpy(module->name, "ipfragment");
	omp_init_lock(&tptd_gparams.ipflock);
	ip_frag_init(nids_params.n_hosts);
	module->pfunction = gen_ip_frag_proc;
	module->dfunction = ip_frag_exit;
