...
float WThreshold = 0.05; //The first node after spliting covers more than a certain part(WThreshold) of all the transactions in the root node
float DiscardThreshold = 0.95; // The number of transactions discarded when a same-level spliting is made must be limited to a certain threshold(DiscardThreshold, otherwise we refuse to make that split
...
{//read the data in a line into a Transaction
	...
	for( int i = 0, j = 0, k = 0; str[i*8] != 10; i ++)
		{
			if(str[i*8] == '!')
				{
				j ++;//row ++
