...
int main(int argc, char * argv[])
{
	...
	fprintf(stderr,"finish reading parameter!\n");
	/*if(argc == 2)
	...
	fprintf(stderr,"finish readfile\n");
		...
		fprintf(stderr,"B\n");
			while(MQueue->QnodeNum)
			{
				...
				for( int i = 0; i < TmpTnode->TLevel; i ++)
					fprintf(fw,"(@%d %d)%s ",TmpTnode->TItem[i].PacketSeqNum, TmpTnode->TItem[i].ByteSeqNum,TmpTnode->TItem[i].Pload);
				fprintf(fw,"\n");
			}
			break;
		}
	...
	fprintf(fw,"parameter: FThreshold: %.2f; WThreshold: %.2f; DiscardThreshold: %.2f; CoverThreshold: %.2f;\n",FThreshold, WThreshold, DiscardThreshold, CoverThreshold);
	fclose(fw);
...
{//read the data in a line into a Transaction
	...
	T[TransactionSeqNum].PacketNum = (int *)malloc(20*sizeof(int));
	T[TransactionSeqNum].ItemArray = (Item **)malloc(20*sizeof(Item *));
	for( int i = 0; i < 20; i ++)
	{
	...
	for( int i = 0, j = 0; str[i*8] != 10 && j < 20; i ++)
		{
			if(str[i*8] == '!')
				{
					j ++;
	...
	for( int i = 0; i < 20 && T[TransactionSeqNum].PacketNum[i]; i ++)
		T[TransactionSeqNum].ItemArray[i] = (Item *)malloc(sizeof(Item) * T[TransactionSeqNum].PacketNum[i]);
	for( int i = 0, j = 0, k = 0; str[i*8] != 10 && j < 20; i ++)
		{
			if(str[i*8] == '!')
				{
				j ++;//row ++
...
{//get all the children of a certain node
	...
	fprintf(stderr,"prepare for step_split_different level\n");
	step_split_differentlevel(MTree, Root, CreatedTnode1, CreatedTnode2);
	fprintf(stderr,"finish step_split_different level\n");
	...
	{//if a spliting is failed, it is based on the rate of the number of children in created1(if the bigger node is too small)
		...
		Root->ChildTnodeNum = 0;
		Root->ChildTnodeArray = 0;
		fprintf(stderr,"return for reason A\n");
		return;
	}
	...
	{//if the bigger node is too big
		...
		fprintf(stderr,"return for reason B\n");
		return;
	}
	Tnode * CreatedTnode3;
	fprintf(stderr,"prepare for step_split_same level\n");
...
/*	for( int i = 0; i < Root->ChildTnodeNum; i ++)
	{
		SignForMatrix = 0;
		for( int j = 0, TotalItemNum = 0; j < Root->ChildTnodeArray[i]->TTransactionNum; j ++)
		{
			for( int l = 0;  l < 20 && Root->ChildTnodeArray[i]->TTransaction[j].PacketNum[l]; l ++)
				TotalItemNum += Root->ChildTnodeArray[i]->TTransaction[j].PacketNum[l];
			if(Root->ChildTnodeArray[i]->TLevel < TotalItemNum)
			{
				SignForMatrix = 1;
				break;
			}
		}
		if(SignForMatrix)//  if the Transactions in this node don't construct a matrix(all the transactions have the same number of items)
			en_queue(Root->ChildTnodeArray[i]);//  then we do enqueue
...
int is_matrix(Tnode * ThisNode)
{
	...
	{
		TotalItemNum = 0;
		for( int l = 0; l < 20 && ThisNode->TTransaction[j].PacketNum[l]; l ++)
